...
export default function DashboardDepartamentoComunicacao() {
    ...
    const loadDashboardData = async () => {
        ...
        try {
            ...
            const [activityData, docStats] = await Promise.all([
                getActivityLogs(10),
                getDocumentStats(),
            ])
    ...
    if (loading) {
        return (
            <DashboardLayout>
                <Header
                    title="A carregar..."
                    subtitle="Dashboard - Departamento de Comunicação"
                />
    ...
    return (
        <DashboardLayout>
            ...
            <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <Card className="relative overflow-hidden border-l-4 border-blue-500">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold text-white">{documentStats?.total || 0}</p>
                            </div>
                        </div>
                    </Card>
                    ...
                    <Card className="relative overflow-hidden border-l-4 border-emerald-500">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-sm text-gray-400">Respondidos</p>
                                <p className="text-2xl font-bold text-white">
                                    {documentStats?.byStatus?.respondido || 0}
                                </p>
                            </div>
                        </div>
                    </Card>
                ...
                {/* Document Types */}
                {documentStats?.byType && Object.keys(documentStats.byType).length > 0 && (
                    <Card>
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-sm font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                        ...
                        <div className="space-y-3">
                            {Object.entries(documentStats.byType)
                                .sort(([, a]: any, [, b]: any) => b - a)
                                .map(([type, count]: any) => {
                                    const pct = Math.round((count / documentStats.total) * 100)
                                    ...
                                    return (
                                        <div key={type}>
                                            <div className="flex items-center justify-between mb-1">
                                                <span className="text-sm text-gray-700 dark:text-gray-300">{typeLabels[type] || type}</span>
                                                <span className="text-sm font-medium text-gray-900 dark:text-white">{count}</span>
                                            </div>
                                            <div className="w-full bg-gray-200 dark:bg-gray-700/50 rounded-full h-2">
                                                <div
                    ...
                    <Link to="/documents">
                        <Card hover className="border-l-4 border-emerald-500">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-white">Gerir Documentos</p>
                                    <p className="text-xs text-gray-400">Criar, editar e gerir documentos</p>
                                </div>
                            </div>
                        </Card>
                    </Link>
