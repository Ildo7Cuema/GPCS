...
import { formatFileSize, rolePermissions } from '../../lib/types'
...
export default function DashboardMunicipal() {
    ...
    const loadDashboardData = async () => {
        ...
        try {
            // As RLS policies já filtram automaticamente por municipio_id
            const [statsData, filesResponse, activityData, actStats, docStats] = await Promise.all([
                getDashboardStats(),
                getMediaFiles({ limit: 6 }),
                getActivityLogs(10),
                getActivityStats(profile.municipio_id),
                getDocumentStats(profile.municipio_id),
            ])
    ...
    if (loading) {
        return (
            <DashboardLayout>
                <Header
                    title="A carregar..."
                    subtitle={`Dashboard Municipal - ${profile?.municipio?.name || ''}`}
                />
    ...
    if (!profile?.municipio_id) {
        return (
            <DashboardLayout>
                <Header
                    title="Sem Município Atribuído"
                    subtitle="Contacte o administrador para atribuir um município"
                />
                <div className="p-6">
                    <Card>
                        <div className="text-center py-8">
                            ...
                            <p className="text-gray-400">
                                O seu perfil não está associado a nenhum município.
                            </p>
    ...
    return (
        <DashboardLayout>
            <Header
                ...
                subtitle={`Dashboard Municipal - ${profile.municipio?.name || 'Município'}`}
            />
                            ...
                            {recentFiles.length === 0 ? (
                                <div className="p-8 text-center">
                                    ...
                                    {permissions.canUpload && (
                                        <Link to="/media" className="text-blue-400 text-sm hover:text-blue-300 mt-2 inline-block">
                            ...
                            <Card hover className="border-l-4 border-blue-500">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-400">Gerir atividades municipais</p>
                                    </div>
                                </div>
                            </Card>
                        </Link>
